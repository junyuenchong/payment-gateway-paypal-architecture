...
export default (): AppConfiguration => {
  ...
  return {
    ...
    bullmq: {
      queues: {
        email:
          optionalString(process.env.BULLMQ_EMAIL_QUEUE_NAME) ?? 'email-queue',
        audit:
          optionalString(process.env.BULLMQ_AUDIT_QUEUE_NAME) ?? 'audit-queue',
        notification:
          optionalString(process.env.BULLMQ_NOTIFICATION_QUEUE_NAME) ??
          'notification-queue',
      },
